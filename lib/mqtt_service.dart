...
class MqttService {
  ...
  // Connection secrets (endpoint, root CA, device cert, private key) live in
  // lib/aws_config.dart, which is git-ignored. See lib/aws_config.example.dart
  // for the template and setup instructions.
  ...
  Future<void> connect() async {
    _client = MqttServerClient.withPort(AwsConfig.host, _clientId, _port);
    _client!.secure = true;
    ...
    ctx.setTrustedCertificatesBytes(utf8.encode(AwsConfig.rootCa));
    ctx.useCertificateChainBytes(utf8.encode(AwsConfig.deviceCert));
    ctx.usePrivateKeyBytes(utf8.encode(AwsConfig.privateKey));
